...
from .data_manager import (
    validate_user_credentials, register_user, get_user_by_email,
    update_password, update_user_details, update_user_religion,
    fetch_sites_by_religion,submit_rating, submit_feedback, get_user_by_id,
    toggle_explore_mode, update_user_address
)
...
class LoginScreen(Screen):
    ...
    def update_translations(self):
        ...
        try:
            ...
            lang = 'en'
        hi = resource_find('data/fonts/NotoSansDevanagari-Regular.ttf')
        kn = resource_find('data/fonts/NotoSansKannada-Regular.ttf')
        en = resource_find('data/fonts/Roboto-Regular.ttf')
        font = {'hi': hi, 'kn': kn}.get(lang, en)
        self.ids.username_input.hint_text = app.get_translations('Enter username')
        self.ids.username_input.font_name = font
        self.ids.password_input.hint_text = app.get_translations('Enter password')
        self.ids.password_input.font_name = font
...
class SiteListScreen(Screen):
    def on_pre_enter(self):
        response = requests.get('http://127.0.0.1:5000/get_sites?religion=No%20Preference')
        self.sites = response.json()
        self.populate_list()

    def populate_list(self):
        container = self.ids.sites_container
        ...
        for site in self.sites:
            btn = Button(
                ...
                size_hint_y=None,
                height=dp(50),
                on_release=lambda btn, s=site: self.go_to_detail(s)
            )
            container.add_widget(btn)

    def go_to_detail(self, site):
        App.get_running_app().selected_site = site
        self.manager.current = 'site_detail'
